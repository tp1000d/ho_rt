...
def _expected_serialized_dtypes():
    """Same dtype map used by deserialize_paths_as_tensor_dicts."""
    paths = {'a': tf.complex64, 'mask': tf.bool, 'normalize_delays': tf.bool,
             'objects': tf.int32, 'phi_r': tf.float32, 'phi_t': tf.float32,
             'reverse_direction': tf.bool, 'sources': tf.float32,
             'targets': tf.float32, 'tau': tf.float32, 'theta_r': tf.float32,
             'theta_t': tf.float32, 'types': tf.int32, 'vertices': tf.float32}
    tmp = {'k_i': tf.float32, 'k_r': tf.float32, 'k_rx': tf.float32,
           'k_tx': tf.float32, 'normals': tf.float32,
           'scat_2_target_dist': tf.float32, 'scat_k_s': tf.float32,
           'scat_last_k_i': tf.float32, 'scat_last_normals': tf.float32,
           'scat_last_objects': tf.int32, 'scat_last_vertices': tf.float32,
           'scat_src_2_last_int_dist': tf.float32, 'sources': tf.float32,
           'targets': tf.float32, 'total_distance': tf.float32,
           'num_samples': tf.int32, 'scat_keep_prob': tf.float32}
    out = {}
    for prefix in ('spec-', 'diff-', 'scat-'):
        out.update({prefix + k: v for k, v in paths.items()})
    for prefix in ('tmp-spec-', 'tmp-diff-', 'tmp-scat-'):
        out.update({prefix + k: v for k, v in tmp.items()})
    out['rx_pos'] = tf.float32
    out['h_meas'] = tf.complex64
    return out
...
def serialize_traced_paths(rx_pos, h_meas, traced_paths, squeeze_target_dim):
    ...
    # Sionna 0.18 returns 8 path objects from trace_paths():
    #   (spec, diff, scat, ris, spec_tmp, diff_tmp, scat_tmp, ris_tmp)
    # This script was written against an older 6-item API (no RIS). Drop the
    # two RIS entries before iterating so the (tmp-spec/diff/scat) blocks
    # downstream end up with the right slices.
    if len(traced_paths) == 8:
        traced_paths = (traced_paths[0], traced_paths[1], traced_paths[2],
                        traced_paths[4], traced_paths[5], traced_paths[6])
    ...
    # drjit 1.x may emit fields with slightly different integer/bool dtypes
    # (e.g. UInt32 instead of Int32 for sample counts). Cast each tensor to
    # the dtype the deserializer expects so parse_tensor doesn't fail.
    _expected = _expected_serialized_dtypes()
    import os as _os
    _dbg = _os.environ.get("SIONNA_SERIALIZE_DEBUG") == "1"
    for k in list(all_tensors.keys()):
        want = _expected.get(k)
        v = all_tensors[k]
        ...
        if want is not None and v.dtype != want:
            try:
                ...
                if _dbg:
                    print(f"  CAST FAIL {k}: {before} -> {want} ({e})")
        if _dbg:
            print(f"  {k}: {before} -> {v.dtype}, shape={tuple(v.shape)}")
        all_tensors[k] = v
